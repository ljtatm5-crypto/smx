...
<html lang="zh-CN">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SMC Admin Panel</title>
<style>
...
<script>
var token='';
function toBJT(iso){if(!iso)return'';var d=new Date(iso);d.setHours(d.getHours()+8);return d.toISOString().slice(0,19).replace('T',' ')}
...
if(t==='users'){var u=await api('/api/admin/users');c.innerHTML='<h3>用户管理</h3><div class=meta>共 '+u.length+' 个用户</div><table><tr><th>用户名</th><th>公钥</th><th>注册时间</th><th>角色</th><th>操作</th></tr>'+(u.length===0?'<tr><td colspan=5 class=empty>暂无用户</td></tr>':u.map(function(x){return'<tr><td>'+x.username+'</td><td style=font-size:11px>'+(x.pubKey==='未设置'?'未设置':x.pubKey.slice(0,20)+'...')+'</td><td>'+toBJT(x.created).slice(0,10)+'</td><td>'+x.role+'</td><td><button class="btn-sm btn-danger" onclick="delUser(\\''+x.username+'\\')">删除</button></td></tr>'}).join(''))+'</table>'}
if(t==='files'){var f=await api('/api/admin/files');c.innerHTML='<h3>文件管理</h3><div class=meta>共 '+f.length+' 个文件</div><table><tr><th>ID</th><th>所有者</th><th>文件名</th><th>大小</th><th>上传时间</th><th>操作</th></tr>'+(f.length===0?'<tr><td colspan=6 class=empty>暂无文件</td></tr>':f.map(function(x){var sz=x.size<1024?x.size+'B':x.size<1048576?(x.size/1024).toFixed(1)+'KB':(x.size/1048576).toFixed(1)+'MB';return'<tr><td style=font-size:11px>'+x.id.slice(0,12)+'...</td><td>'+x.owner+'</td><td>'+x.name+'</td><td>'+sz+'</td><td>'+toBJT(x.uploadedAt).slice(0,10)+'</td><td><button class="btn-sm btn-danger" onclick="delFile(\\''+x.id+'\\')">删除</button></td></tr>'}).join(''))+'</table>'}
if(t==='logs'){var l=await api('/api/admin/logs');c.innerHTML='<h3>审计日志</h3><table><tr><th>时间</th><th>用户</th><th>操作</th><th>详情</th></tr>'+(l.length===0?'<tr><td colspan=4 class=empty>暂无日志</td></tr>':l.map(function(x){return'<tr><td>'+toBJT(x.createdAt)+'</td><td>'+x.username+'</td><td>'+x.action+'</td><td>'+x.detail+'</td></tr>'}).join(''))+'</table>'}
}
...
async function startServer(){
  ...
  app.get('/api/files/:id/download',userAuth,(req,res)=>{
    ...
    if(!r.length||!r[0].values.length){r=db.exec("SELECT f.* FROM files f JOIN shares s ON f.id=s.file_id WHERE f.id=? AND s.username=?",[req.params.id,req.user])}
    if(!r.length||!r[0].values.length)return res.status(404).json({error:'not found'})
    const fp=path.join(STORAGE_DIR(),req.params.id+'.enc');if(!fs.existsSync(fp))return res.status(404).json({error:'file lost'});res.setHeader('Content-Disposition','attachment; filename="'+encodeURIComponent(r[0].values[0][2])+'"');addLog(req.user,'文件下载',r[0].values[0][3]||r[0].values[0][2],'SM2验签下载');res.sendFile(fp)
  })
